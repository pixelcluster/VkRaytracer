...
HardwareRaytracer::HardwareRaytracer(size_t windowWidth, size_t windowHeight)
	...
	extensionNames.reserve(requiredExtensionCount + m_enableDebugUtils);

	for (uint32_t i = 0; i < requiredExtensionCount; ++i) {
		extensionNames.push_back(extensions[i]);
	}

	if constexpr (m_enableDebugUtils) {
		extensionNames.push_back(VK_EXT_DEBUG_UTILS_EXTENSION_NAME);
	}

	std::vector<const char*> instanceLayerNames;
	instanceLayerNames.reserve(m_enableValidation);

	if constexpr (m_enableValidation) {
	...
	for (auto& device : devices) {
		...
		VkPhysicalDeviceAccelerationStructureFeaturesKHR accelerationStructureFeatures = {
			.sType = VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_ACCELERATION_STRUCTURE_FEATURES_KHR, .pNext = &rayTracingFeatures
		};
	...
	const char* deviceExtensionNames[] = { VK_KHR_ACCELERATION_STRUCTURE_EXTENSION_NAME,
	...
	VkDeviceCreateInfo deviceCreateInfo = { .sType = VK_STRUCTURE_TYPE_DEVICE_CREATE_INFO,
											...
											.enabledLayerCount = static_cast<uint32_t>(instanceLayerNames.size()),
											.ppEnabledLayerNames = instanceLayerNames.data(),
											.enabledExtensionCount = sizeof(deviceExtensionNames) / sizeof(const char*),
	...
	for (size_t i = 0; i < m_frameInFlightCount; ++i) {
	...
	m_swapchain = createSwapchain(m_physicalDevice, m_device, m_surface,
								  ...
								  VK_NULL_HANDLE, m_enableDebugUtils);

	createSwapchainResources();
}

HardwareRaytracer::~HardwareRaytracer() {
	VkFence fences[m_frameInFlightCount];
	for (size_t i = 0; i < m_frameInFlightCount; ++i) {
		fences[i] = m_perFrameData[i].fence;
	}
	verifyResult(vkWaitForFences(m_device, m_frameInFlightCount, fences, VK_TRUE, UINT64_MAX));
...
bool HardwareRaytracer::update() {
	...
	if (m_window.windowSizeDirty() || !m_isSwapchainGood) {
		...
		m_swapchain =
			createSwapchain(m_physicalDevice, m_device, m_surface,
							{ static_cast<uint32_t>(m_window.width()), static_cast<uint32_t>(m_window.height()) },
							m_swapchain, m_enableDebugUtils);
		createSwapchainResources();
	}

	VkResult acquireResult =
		vkAcquireNextImageKHR(m_device, m_swapchain, UINT64_MAX,
							  m_perFrameData[m_currentFrameIndex].acquireDoneSemaphore, VK_NULL_HANDLE, &imageIndex);
	...
	VkImage currentSwapchainImage = m_swapchainImages[imageIndex];
	...
	VkImageMemoryBarrier memoryBarrierBefore = { .sType = VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER,
												 ...
												 .srcQueueFamilyIndex = m_queueFamilyIndex,
												 .dstQueueFamilyIndex = m_queueFamilyIndex,
												 .image = currentSwapchainImage,
												 .subresourceRange = imageRange };

	vkCmdPipelineBarrier(m_perFrameData[m_currentFrameIndex].commandBuffer, VK_PIPELINE_STAGE_TRANSFER_BIT,
						 VK_PIPELINE_STAGE_TRANSFER_BIT, 0, 0, nullptr, 0, nullptr, 1, &memoryBarrierBefore);

	VkClearColorValue clearValue = { .float32 = { 0.7f, 0.2f, 0.7f, 1.0f } };

	vkCmdClearColorImage(m_perFrameData[m_currentFrameIndex].commandBuffer, currentSwapchainImage,
						 VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, &clearValue, 1, &imageRange);
	...
	vkCmdPipelineBarrier(m_perFrameData[m_currentFrameIndex].commandBuffer, VK_PIPELINE_STAGE_TRANSFER_BIT,
						 VK_PIPELINE_STAGE_TRANSFER_BIT, 0, 0, nullptr, 0, nullptr, 1, &memoryBarrierAfter);
	...
	++m_currentFrameIndex %= m_frameInFlightCount;
...
void HardwareRaytracer::createPerFrameData(size_t index) {
	VkCommandPool pool;
...
void HardwareRaytracer::createSwapchainResources() {
	for (auto& view : m_swapchainViews) {
		vkDestroyImageView(m_device, view, nullptr);
	}
...
bool HardwareRaytracer::canRecreateSwapchain() {
