...
int main() {
	...
	std::vector<Sphere> spheres = std::vector<Sphere>(2 * 2);
	...
	for (size_t i = 0; i < 2; ++i) {
		for (size_t j = 0; j < 2; ++j) {
			float floatIndex = static_cast<float>(i) * 2.0f + static_cast<float>(j) + 0.8;
			spheres[j + i * 2] = Sphere{ .position = { 2.0f * i, -2.0f, 2.0f * j, 1.0f },
										 .radius = 2.0f,
	...
	while (raytracer.update(spheres)) {
		 for (size_t i = 0; i < 2; ++i) {
			for (size_t j = 0; j < 2; ++j) {
				...
				float colorLength = sqrtf(.8f * .8f + .5f * .5f + .3f * .3f);
				spheres[j + i * 2] = Sphere{ .position = { 2.0f * i, -y, 2.0f * j, 1.0f },
											 .radius = 0.5f,
					...
					{ 0.8f / colorLength, 0.5f / colorLength, 0.3f / colorLength, 1.0f }
				};
				if (j + i == 0 || (j + i * 2) == 3) { // if ((j + i * 50) % 2) {
					//negative color.a indicates radiance
					spheres[j + i * 2].position[0] += 10.0f;
					spheres[j + i * 2].position[1] -= 3.4f;
					spheres[j + i * 2].position[2] += 10.0f;
					spheres[j + i * 2].color[3] = -50.0f;
				}
			}
		}
	}
