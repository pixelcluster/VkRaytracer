...
vec3 weightLight(bool sampledHemisphere, LightData lightData, vec3 hitPoint, vec3 sampleDir, vec3 objectHitNormal, vec4 radiance) {
	...
	return (abs(lightBSDFFactor * dot(gl_WorldRayDirectionEXT, objectHitNormal)) * (radiance.rgb * radiance.a) / max(lightPdf, 0.00001f)) * powerHeuristic(1, lightPdf, 1, bsdfPdf);
}
